...
| `SUPABASE_URL` | for self-serve | Supabase project URL → verifies session JWTs via JWKS (**ES256/RS256**, the modern default); preferred |
| `SUPABASE_JWT_SECRET` | legacy only | Verifies **HS256** session JWTs (older projects on the shared secret); used only if `SUPABASE_URL` is unset |

**Self-serve auth verification:** the API verifies the dashboard's Supabase session tokens. Supabase now
signs with **asymmetric keys (ES256 P-256 / RS256)** — set **`SUPABASE_URL`** (e.g. `https://<ref>.supabase.co`)
and the API fetches the project JWKS (`/auth/v1/.well-known/jwks.json`). Only fall back to
`SUPABASE_JWT_SECRET` if your project still signs with the legacy HS256 shared secret.

**Dashboard self-serve env** (Vercel, `apps/client-dashboard`): set `NEXT_PUBLIC_SUPABASE_URL` +
`NEXT_PUBLIC_SUPABASE_ANON_KEY` to enable the `/signup` page (rebuild after). Use the **same** Supabase
project as the API's `SUPABASE_URL`.
